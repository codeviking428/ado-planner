...
import { useGantt, useGanttSelector, useGanttViewConfig } from '@/components/reui/gantt/gantt'
import { useGanttGestures, wasRecentDrag } from '@/components/reui/gantt/gantt-dnd'
import { flattenResources, toZoned } from '@/components/reui/gantt/gantt-lib'
import type { GanttOccurrence, GanttSegment } from '@/components/reui/gantt/gantt-types'
...
function GanttBar<TData = unknown>({
  ...
  const defaultProps = {
    ...
    className: cn(
      'group/gantt-bar-group @container relative flex w-full min-w-0 cursor-pointer touch-none items-center gap-1.5 overflow-hidden rounded-sm px-1.5 py-0.5 text-start leading-normal text-foreground select-none',
